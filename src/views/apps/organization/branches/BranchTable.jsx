...
const BranchTable = () => {
    ...
    // RTK Query
    const { data: detail_data, isFetching, refetch } = useGetBranchesQuery({
        page: page + 1,
    ...
    const branchSchema = z.object({
        ...
        phone_number: z.string({ required_error: "Phone Number is Required!" }).regex(/^\+?\d{1,4}?[\d\s\-\(\)]{7,15}$/, "Invalid phone number!"),
                ...
                cell: ({ row }) => (
                    <div className="flex items-center w-fit gap-2">
                        ...
                        <Tooltip title="Change Password" placement="top">
                            <IconButton >
                                <FaLock className='cursor-pointer text-base text-primary'
                                    onClick={() => {
