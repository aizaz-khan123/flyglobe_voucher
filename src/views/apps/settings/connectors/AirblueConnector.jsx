...
import { Card, CardContent, FormControlLabel, FormLabel, Switch } from '@mui/material'
...
const AirblueConnector = () => {
  const { data: supplierDropDown } = useGetSupplierListQuery()
  const { data: connector, isSuccess: isConnectorSuccess } = useShowConnectorQuery('AIRBLUE_API')
  const [updateConnector, { isLoading: isUpdateLoading }] = useUpdateConnectorMutation()

  const connectorSchema = z.object({
    api_key: z.string({ required_error: 'API_ID Required!' }),
    api_secret: z.string({ required_error: 'API_PASSWORD Required!' }),
    connector_domain: z.string({ required_error: 'API Client ID/KEY Required!' }),
  ...
  const onSubmit = handleSubmit(async data => {
    ...
    await updateConnector(updated_data).then(response => {
      if (response.data?.code == 200) {
        toast.success('AIRBLUE API Credentials Updated!')
      }
    })
  })
      ...
      <Card className='mt-5 bg-base-100'>
        <CardContent className={'p-0'}>
          <CardContent className='gap-0'>
            <div>AIRBLUE API Credentials</div>
            <div className='mt-1 grid grid-cols-1 gap-5 gap-y-3 md:grid-cols-2'>
              <div>
                <FormLabel title={'API ID'} htmlFor='api_key'></FormLabel>
                <MuiTextField
              ...
              <div>
                <FormLabel title={'API PASSWORD'} htmlFor='api_secret'></FormLabel>
                <MuiTextField
                  className='w-full border-0 focus:outline-0'
                  control={control}
                  size='md'
              ...
              <div>
                <FormLabel title={'APIClient ID/key'} htmlFor='connector_domain'></FormLabel>
                <MuiTextField
                  className='w-full border-0 focus:outline-0'
                  control={control}
                  size='md'
                  ...
                  placeholder='Enter APIClient ID/key'
                />
              </div>
              <div>
