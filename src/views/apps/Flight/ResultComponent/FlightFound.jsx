...
const FlightFound = () => {
  ...
  useEffect(() => {
    ...
    const filtered = mergeFlightResponse.filter(flight => {
      ...
      // New airline matching logic
      // const matchesAirline = () => {
      //   // if (isRoundtrip) {
      //   const flightSector = normalizedLegs?.[0]?.sector?.join('-');
      //   if (flightSector === returnSector) {
      //     const selectedAirline = selectedAirlinesBySector[outgoingSector];
                      ...
                      <Card key={index} className='mb-5 static'>
                        <CardContent>
                          ...
                          {normalizedLegs.map((leg, legIndex) => {
                            ...
                            const isOutbound = legIndex === 0; // Outbound flight is the first leg
                            // console.log('legIndex', legIndex);
                            // console.log('sectorKey', sectorKey);
                            // console.log('isOutbound', isOutbound);


                            const isInbound = legIndex === 1; // Inbound flight is the second leg
                            // console.log('isInbound', isInbound);
                            const isOutboundSelected = Object.values(selectedFares).some(
                              (fare) => fare.sector === sectorKey && isOutbound
                            );
                            console.log('isOutboundSelected', isOutboundSelected);

                            // Show outbound accordion only if no outbound fare is selected
                            if (isOutbound && isOutboundSelected) return null;

                            // Show inbound accordion only if an outbound fare is selected
                            if (isInbound && !isOutboundSelected) return null;
                                ...
                                <Accordion className='col-span-12 mb-3'>
                                  ...
                                  <AccordionDetails className='grid grid-cols-12 gap-4 pt-4'>
                                    {[
                                      ...
                                      ...(Array.isArray(data?.fare_option) ? data.fare_option : [])
                                    ]?.filter((faresGroupData) => {
                                      // Filter inbound flights to match the selected airline
                                      if (isInbound) {
                                        const selectedAirline = Object.values(selectedFares)[0]?.airline?.name;
                                        return faresGroupData.airline?.name === selectedAirline;
                                      }
                                      return true;
                                      ...
                                      const grossAmount =
                                        Number(faresGroupData?.price?.gross_amount.replace(/,/g, '')) || 0
                                      const totalFare = grossAmount
                                      const formattedTotalFare = totalFare.toLocaleString()

                                      const isSelected =
                                        selectedFares[legIndex]?.booking_id === faresGroupData.booking_id;
                                          ...
                                          >
                                            ...
                                            <div className='px-4 bg-[#F5F6FF]'>
                                              ...
                                              <div className='mt-4 flex flex-col items-end'>
                                                ...
                                                {data.provider === 'AIRBLUE_API' ? <Button
                                                  variant='contained'
                                                  className='w-full'
                                                  disabled={isSelected}
                                                  onClick={() => {
                                                    const sector = leg.sector.join('-');
                                                    handleFareSelect(legIndex, {
                                                      ...faresGroupData,
                                                      sector,
                                                      airline: data.airline, // Pass airline information
                                                    });
                                                  }}
                                                >
                                                  {selectedFares[legIndex]?.booking_id === faresGroupData.booking_id
