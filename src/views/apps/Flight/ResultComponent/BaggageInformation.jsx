      ...
      {fareOptions.map((fare, index) => (
        <div key={index} className='p-4 border rounded-lg'>
                ...
                return (
                  <div key={passengerType}>
                    ...
                    <div className='space-y-1'>
                      {checkedBaggage && <div className='text-sm'>{checkedBaggage}</div>}
                      {cabinBaggage && <div className='text-sm'>{cabinBaggage}</div>}
                    </div>
                  </div>
                )
              })}
          ...
          <div className='mt-2 text-sm'>
            <div className='mb-1'>
              <strong>Base Fare:</strong> {fare.price.currency} {fare.price.base_fare} + <strong>Tax:</strong>{' '}
              {fare.price.currency} {fare.price.tax}
            </div>
