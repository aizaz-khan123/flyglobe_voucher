    ...
    <div className='space-y-4'>
      {fareOptions.map((fare, index) => (
        <div key={index} className='p-4 border rounded-lg'>
          ...
          <div
            className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${fare.is_refundable ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
          >
          ...
          <div className='mt-2 text-sm'>
            <div className='mb-1'>
              <>Base Fare:</> {fare.price.currency} {fare.price.base_fare} + <>Tax:</> {fare.price.currency}{' '}
              {fare.price.tax}
            </div>
