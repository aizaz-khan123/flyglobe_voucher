    ...
    <Box>
      <Card sx={{ mb: 5 }}>
        <CardContent>
          ...
          {Object.entries(bookingAvailabilityConfirmationData?.price?.fare_break_down || {}).map(
            ([key, data], index) => (
              <Box
                key={index}
                sx={{
          ...
          <Box
            sx={{ display: 'flex', justifyContent: 'space-between', mt: 2, borderBottom: '1px solid #e0e0e0', pb: 1 }}
          >
            <Typography fontWeight='600'>Price you Pay:</Typography>
            <Typography fontWeight='bold' sx={{ color: 'primary.main' }}>
              {bookingAvailabilityConfirmationData?.price.currency}{' '}
              {bookingAvailabilityConfirmationData?.price.gross_amount}
            </Typography>
          </Box>
