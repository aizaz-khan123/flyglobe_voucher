...
import { Accordion, AccordionDetails, AccordionSummary, Button, Card, CardContent, Dialog, DialogActions, DialogContent, Table, TableBody, TableHead, TableRow } from "@mui/material";
...
import { useBookingByIdQuery, useBookingCancelMutation, useDownloadBookingMutation, useTicketOtpEmailMutation } from "@/redux-store/services/api";
...
const BookingDetail = ({ bookingId }) => {
    ...
    const { passengers, booking_pnr, pnr_expiry, airline, gross_fare, fare_break_down, booking_brand, status, booked_segments, baggage, issued_at } = booking;
                                ...
                                <div className="mt-1 space-y-1  max-w-full overflow-x-auto max-h-[400px] border rounded-lg">
                                    <Table className="min-w-max text-start">
                                        <TableHead className="bg-gray-200 text-start">
                                            <th className="text-sm font-semibold text-gray text-start p-3">
                                                Passenger Name.
                                            </th>
                                            <th className="text-sm font-semibold text-gray text-start p-3">
                                                Date of Birth.
                                        ...
                                        <TableBody hasData={true}>
                                            {passengers?.map((passenger, index) => (
                                                <tr key={index} className="hover:bg-base-200/40 text-base font-semibold text-gray-800 border-b">
                                                    <td className="p-3">{passenger?.title}. {passenger?.first_name} {passenger?.last_name}</td>
                                                    <td className="p-3">{passenger?.date_of_birth}</td>
                                                    <td className="p-3">{passenger?.d_type === 'P' ? 'Passport' : passenger?.d_type === 'I' ? 'CNIC' : passenger?.d_type}</td>
                ...
                <div className="col-span-12 md:col-span-3">
                    ...
                    <Card className="bg-base-100 mb-5">
                        ...
                        <CardContent>
                            ...
                            >
                                <AccordionSummary
                                    expandIcon={<div className="bg-primary text-white rounded-full w-8 h-8 flex items-center justify-center"><ExpandMoreIcon /></div>}
                            ...
                            {isConfirmed && (
                                <div className="flex justify-center flex-col space-y-4 mt-4">
                                    {isPNRValid && (
                                        <>
                                            {(
                                                ...
                                                ((role === "agency" || role === "a-employee" || role === "sub-agent") && setting?.can_issue_ticket)
                                            ) && (
                                                    <Button
                                                        variant="contained"
                                                        startIcon={<FaTicket />}
                                    ...
                                    <Button variant="contained" startIcon={<TbBrandBooking className="text-2xl" />} onClick={handleEmailBookingModal}>Email Booking</Button>
                                    <Button variant="contained" startIcon={<FaDownload />} onClick={() => handleDownloadTicket('booking_download')} disabled={downloadLoading} loading={downloadLoading}>Download Booking</Button>
                            ...
                            {status == 'issued' && (
                                <div className="flex justify-center flex-col space-y-4 mt-4">
                                    <Button variant="contained" startIcon={<FaDownload />} onClick={() => handleDownloadTicket('ticket_download')} disabled={downloadLoading} loading={downloadLoading}>
                                        Download Ticket
                                    </Button>
                                    {isWithin24Hours ? (
                                        <Button variant="contained" startIcon={<FaTicket />} onClick={() => setIsVoidModalOpen(!isVoidModalOpen)}>Void Ticket</Button>
                                    ) : (
                                        <Button variant="contained" startIcon={<RiRefund2Fill />} onClick={handleRefundTicket}>Refund Ticket</Button>
                                    )}
                                    <Button variant="contained" startIcon={<FaTicket />} onClick={handleEmailTicketModal}> Email Ticket</Button>
                                </div>
                            )}
                    ...
                    <Dialog open={openCancelModal}>
                        ...
                        <DialogActions>
                            ...
                            <form method="dialog">
                                <Button loading={isBookingCancelLoading} disabled={isBookingCancelLoading} color="primary" size="sm" onClick={() => handleCancelBooking()}>
                                    Yes
                                </Button>
                            </form>
                        </DialogActions>
