        ...
        render={({ field }) => (
          <DateRangePicker
            ...
            onChange={newValue => {
              ...
              const formattedReturn = disableEndDate || !newValue[1] ? null : newValue[1].format('YYYY-MM-DD')
            ...
            shouldDisableDate={(date, position) =>
              position === 'start' ? minDate && date.isBefore(minDate, 'day') : disableEndDate
            }
            minDate={minDate}
            maxDate={maxDate}
            slotProps={{
              textField: ({ position }) => ({
                className: `${className} ${
                  disableEndDate && position === 'end' ? 'bg-gray-200 cursor-not-allowed' : ''
                }`,
                fullWidth: true,
                size,
                InputProps: {
